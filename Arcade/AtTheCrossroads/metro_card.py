...
Example
    ...
    There are 30 days in April, June, September and November, so the next
    months to consider are May, July, October or December. All of them have
    exactly 31 days, which means that you will definitely get a 31-days pass
    the next time you extend your card.

Input/Output

    [input] integer lastNumberOfDays

        A positive integer, the number of days for which the card was extended
        the last time. This number can be equal to 28, 30 or 31.
...
def metroCard(lastNumberOfDays):
    #December, being the 12th month, can also be thought of as the
    #0th month, so start there for modding purposes
    days = [31, 31, 28, 31, 30, 31, 30, 31, 31, 30, 31, 30]

    options = set()

    for month_num in range(12):
        #figure out the month num for the next month
        next_month_num = (month_num + 1) % 12

        #if this month could have been the current one
        if days[month_num] == lastNumberOfDays:
            #then next month is an option
            options.add(days[next_month_num])
