...
Example
    ...
    Let's say that there are 4 students, and the second one can't tell left
    from right. In this case, only after the second, third and fifth commands
    will the students face the same direction.
...
def lineUp(commands):
    count = 0
    facing_the_same_way = True
    for command in commands:
        if ((facing_the_same_way and command in ["L", "R"]) or
            (not facing_the_same_way and command == "A")):
            facing_the_same_way = False
        else:
            facing_the_same_way = True
            count += 1
